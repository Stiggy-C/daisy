...
import io.openenterprise.daisy.example.data.Gender;
import io.openenterprise.daisy.example.data.MemberTier;
...
public class MemberDataGenerator {
    ...
    public static final String DB_TABLE = "members";
    ...
    public void generate(@Nonnull List<String> memberIds) {
        ...
        jdbcTemplate.execute("CREATE TABLE IF NOT EXISTS " + DB_TABLE + " (\n" +
        ...
        var batchArgs = memberIds.stream()
                .map(memberId -> {
        ...
        var insertSql = "INSERT INTO " + DB_TABLE + " VALUES (?, ?, ? ,? , ?, ?, ?, ?, ?)";
